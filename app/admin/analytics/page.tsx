    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6">
          ...
          <div className="text-3xl font-bold text-white">{stats.recentViews}</div>
        </div>
        <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6">
        ...
        <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6">
          ...
          <div className="text-3xl font-bold text-white">{stats.projectViews}</div>
        </div>
      </div>
            ...
            {stats.topBlogPosts.map((post: any, idx: number) => (
              <div key={idx} className="flex items-center justify-between">
                <div className="flex-1 min-w-0">
                  <p className="text-white truncate">{post.title || 'Untitled'}</p>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-20 h-2 bg-white/10 rounded-full overflow-hidden">
                    <div
                      className="h-full bg-gradient-to-r from-purple-600 to-blue-600"
                      style={{ width: `${(post.count / stats.topBlogPosts[0].count) * 100}%` }}
                    />
                  </div>
                  <span className="text-gray-400 text-sm w-12 text-right">{post.count}</span>
                </div>
              </div>
            ))}
          </div>
